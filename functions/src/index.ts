...
export const CheckRoomRequest = functions.database.ref('/requests/{userId}')
.onWrite(event => {
...
*/
export const parseVideoUrlNew = functions.database.ref('/online/{roomId}/{userId}')
.onWrite(event => {
  if(!event.data.exists()) return false;
  if(event.data.val().status > 0) return false; 
  return admin.database().ref(`/rooms/${event.params.roomId}`).once('value', (room) => {
...
function parseUrl(event, room) {
  ...
  if (hostname === "www.ts.kg") {
    ...
    return request(options, (error, response, body) => {
      if (error) console.log('tskg request E: '+ error);
    ...
    return admin.database().ref(`/online/${room.key}/${event.params.userId}`).once('value',user => {
      ...
      return request(options, (error,response,body)=> {
        if (error) console.log('ockg request E: '+ error);
        const data = JSON.parse(body)
        if (data.success){
          return admin.database().ref(`/player/${room.key}/video_url/${event.params.userId}`).set(data.data)
        }
...
app.get('/getip', (req, res) => {
  ...
  if (ip) res.send({success:true, code:200, data:{ip, user_agent}})
  else res.send({success:false, code:400})
});
